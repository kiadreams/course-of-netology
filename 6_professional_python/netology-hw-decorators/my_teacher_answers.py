...
def logger_v2(old_function):
    ...
    handler = RotatingFileHandler(path, backupCount=10,
                                  maxBytes=1000000,
                                  encoding='utf-8')
    ...
    def new_function(*args, **kwargs):
        ...
        __logger.debug(
            f'Вызвана функция {old_function.__name__} c аргументами {
                args} и {kwargs} Результат: {result}'
        )
...
def test():
    ...
    loggers = (
        logger_v1, logger_v2,
        logger_v3('log_1.log'),  logger_v3(
            'log_2.log'), logger_v3('log_3.log'),
